...
//FUNKTION DIE EINE ZUFÄLLIGE ZAHL ZURÜCKGIBT. Nicht von mir geschrieben (Quelle:https://developer.mozilla.org/de/docs/Web/JavaScript/Reference/Global_Objects/Math/math.random)
...
//Sämtlicher nachfolgender Code wurde nur von mir verfasst

//Diese Funktion realisiert das Aus- und Einklappen des Menüs 
var menustate: boolean = true;
function togglemenu(): void {
    if (menustate) {
        Velocity(document.getElementsByClassName("navigationbox"), "transition.slideDownBigIn",
            { stagger: 250, drag: true, duration: 500, delay: 100 });
        menustate = !menustate;
    }
    else {
        Velocity(document.getElementsByClassName("navigationbox"), "transition.slideUpBigOut",
            { stagger: 350, backwards: true, drag: true, duration: 500, delay: 100 });
        menustate = !menustate;
    }
}

//Funktion die zurückggibt ob man sich in der mobilen Ansicht befindet
...
//Funktion die sämtliche Headeranimationen ausführt
//1. Headertexte werden durchgewechselt
//2. Auf Grundlage einer zufälligen Zahl werden verschiedene Unterüberschriften und dazugehörige Texte für den Scrolltrigger angezeigt.
function headeranimation(): void {
    ...
    for (i = 1; i < headertext.length; i++) {
        Velocity(headertext[i], "transition.slideDownBigIn",
            { duration: 650, delay: animationdelay });

        if (i + 1 < headertext.length) {
            Velocity(headertext[i], "transition.slideDownBigOut",
    ...
    //ANIMATIONEN DER UNTERÜBERSCHRIFT UND DES SCROLLINGTRIGGERS (ZUFÄLLIG)
    ...
    switch (subscroll) {
        ...
        case 3:
            ...
            break;
        default: document.getElementById("headersub").innerHTML = sub1;
            document.getElementById("scrolltext").innerHTML = note1;
...
document.addEventListener('DOMContentLoaded', function() {
    var newState: boolean = isMobile();

    //Optimierter Resize Event Listener und Funktion welche die Seite beim Resizen neu lädt
    ...
    //ALLE ANIMATIONEN DIE NUR IN DER DESKTOPANSICHT AUSGEFÜHRT WERDEN
    ...
    //EVENTLISTENER FÜR MOBIL UND DESKTOP 

    //EVENTLISTENER FÜR AUSKLAPPEN/EINKLAPPEN DES MENÜS
    document.getElementById("menubutton").addEventListener("click", togglemenu);



    //EVENTLISTENER FÜR DAS AUSKLAPPMENÜ
    ...
    //EVENTLISTENER FÜR MOBILE STARTSEITE
